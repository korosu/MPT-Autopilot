...
def list_bgm_files(mpt_songs_dir: Path, filter_str: str = "") -> list[tuple[str, int]]:
    """Return sorted list of (filename, size_bytes) for .mp3 files from resource/songs/."""
    if not mpt_songs_dir.exists():
        return []
    files = [(f.name, f.stat().st_size) for f in mpt_songs_dir.glob("*.mp3")]
    if filter_str:
        fl = filter_str.lower()
