...
def test_quota_exceeded_is_detected(monkeypatch):
    monkeypatch.setattr(
        "mpt_autopilot.uploader.uploader.subprocess.run",
...
def test_retries_then_eventually_succeeds(monkeypatch):
    ...
    monkeypatch.setattr("mpt_autopilot.uploader.uploader.subprocess.run", fake_run)
    output = upload_video(
        Path("fake"),
        Path("v.mp4"),
        Path("m.json"),
...
def test_oserror_chained_as_cause(monkeypatch):
    ...
    monkeypatch.setattr("mpt_autopilot.uploader.uploader.subprocess.run", boom)
    with pytest.raises(UploadFailed) as exc_info:
        upload_video(Path("fake"), Path("v.mp4"), Path("m.json"), Path("s"), Path("t"))
